  ...
  useEffect(() => {
    ...
    void fetchAssets('audioBar')
      ...
      .catch(err => {
        if (!cancelled) {
          setItems([]);
          setError(err instanceof Error ? err.message : 'Không thể tải danh sách phổ âm thanh');
        }
      })
      .finally(() => {
      ...
      >
        <div className='space-y-4'>
          ...
          {!loading && items.length === 0 ? (
            <p className='text-center text-xs text-neutral-500'>Chưa có video phổ âm thanh trong assets</p>
          ) : null}
        </div>
      </Modal>
