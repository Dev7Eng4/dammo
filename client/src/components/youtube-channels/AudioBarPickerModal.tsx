...
interface AudioBarPickerModalProps {
  open: boolean
  onClose: () => void
  selectedFile: string
...
export function AudioBarPickerModal({ open, onClose, selectedFile, onSelect }: AudioBarPickerModalProps) {
  ...
  useEffect(() => {
    ...
    void fetchAssets('audioBar')
      ...
      .catch(err => {
        if (!cancelled) {
          setItems([])
          setError(err instanceof Error ? err.message : t('picker.audioBarLoadError'))
        }
      })
      .finally(() => {
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button
              ...
              className={`rounded-lg ${
                autoSelected ? 'border-emerald-500 bg-emerald-500/15 text-emerald-300' : ''
              }`}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
              {items.map(item => {
                ...
                const src = assetFileUrl('audioBar', item.name)
                return (
                  <div
                    key={item.name}
            ...
            <p className="text-center text-xs text-neutral-500">{t('picker.audioBarEmpty')}</p>
          )}
        </div>
      </Modal>
