  ...
  useEffect(() => {
    ...
    void fetchAssets('subscribe')
      ...
      .catch(err => {
        if (!cancelled) {
          setItems([])
          setError(err instanceof Error ? err.message : t('picker.subscribeLoadError'))
        }
      })
      .finally(() => {
      ...
      >
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <p className="text-center text-xs text-neutral-500">{t('picker.subscribeEmpty')}</p>
          )}
        </div>
      </Modal>
