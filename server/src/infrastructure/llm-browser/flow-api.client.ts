...
export function buildBatchGeneratePayload(options: BuildBatchGeneratePayloadOptions): Record<string, unknown> {
  ...
  const imageInputs = primaryMediaId
    ? [{ imageInputType: 'IMAGE_INPUT_TYPE_REFERENCE', name: primaryMediaId }]
    : [];
...
export async function getAccessTokenFromPage(page: Page, sessionUrl = FLOW_SESSION_URL): Promise<string> {
  try {
    const session = await page.evaluate(async (url: string) => {
      ...
      return res.json() as Promise<{ access_token?: string; accessToken?: string } | null>;
    }, sessionUrl);
...
export async function uploadReferenceImageViaApi(
  accessToken: string,
  imagePath: string,
  projectId: string,
): Promise<string | null> {
...
export function assertFlowApiResponseOk(result: CallBatchGenerateImagesResult): void {
  ...
  if (result.status === 403) {
    message +=
      '. Possible causes: expired access token, invalid reCAPTCHA token, or outdated x-browser-validation request headers.';
