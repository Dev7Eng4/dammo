...
import {
  ...
  deleteCelebrityMediaSchema,
  updateCelebritySchema,
} from './celebrities.schema.js';
import { celebritiesService } from './celebrities.service.js';
...
export function createCelebritiesRoutes() {
  const app = new Hono();

  app.get('/', (c) => c.json({ items: celebritiesService.list() }));

  app.post('/', zValidator('json', createCelebritySchema), (c) => {
    const body = c.req.valid('json');
    const item = celebritiesService.create(body);
    return c.json({ item }, 201);
  });

  app.get('/:id/media', (c) => {
    const items = celebritiesService.listMedia(c.req.param('id'));
    return c.json({ items });
  });
  ...
  app.post('/:id/media', async (c) => {
    ...
    const item = await celebritiesService.uploadMedia(id, file);
    return c.json({ item }, 201);
  });

  app.get('/:id/media/:filename', (c) => {
    const asset = celebritiesService.getMediaFile(c.req.param('id'), c.req.param('filename'));
    return streamMediaFile(c.req.raw, asset);
  });

  app.delete('/:id/media', zValidator('json', deleteCelebrityMediaSchema), (c) => {
    const body = c.req.valid('json');
    const result = celebritiesService.deleteMedia(c.req.param('id'), body.names);
    return c.json(result);
  });

  app.get('/:id', (c) => {
    const item = celebritiesService.getById(c.req.param('id'));
    return c.json({ item });
  });

  app.patch('/:id', zValidator('json', updateCelebritySchema), (c) => {
    const body = c.req.valid('json');
    const item = celebritiesService.update(c.req.param('id'), body);
    return c.json({ item });
  });

  app.delete('/:id', (c) => {
    celebritiesService.delete(c.req.param('id'));
