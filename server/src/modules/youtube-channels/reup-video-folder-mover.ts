...
import type { ReupVideoOutputItem } from './reup-video.types.js';
...
export async function moveVideoFolderToChannel(channelId: string, youtubeVideoId: string): Promise<string> {
  const sourceDir = mediaDownloadDir('youtube', youtubeVideoId);
  const destDir = youtubeChannelVideoDir(channelId, youtubeVideoId);
