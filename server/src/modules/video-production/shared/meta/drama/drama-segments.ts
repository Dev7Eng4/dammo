...
/** Long-path segment window length. */
...
/** Stride between window starts (= segment − overlap). */
export const DRAMA_STRIDE_MS = DRAMA_SEGMENT_MS - DRAMA_OVERLAP_MS;
 ...
 * - >40 min → 28-min windows with 2-min overlap (stride 26 min)
 */
export function buildDramaSegments(blocks: SrtBlock[], durationMs: number): DramaTranscriptSegment[] {
  ...
  while (startMs < durationMs) {
    const endMs = Math.min(startMs + DRAMA_SEGMENT_MS, durationMs);
